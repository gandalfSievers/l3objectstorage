...
use crate::types::response::{GetObjectTaggingResponse, TagSetXml};
use crate::utils::xml::to_xml;

/// Handle GetObjectTagging request
pub async fn get_object_tagging(
    ...
    let tags = storage.get_object_tags_versioned(bucket, key, version_id).await?;

    let response_body = GetObjectTaggingResponse {
...
/// Handle PutObjectTagging request
pub async fn put_object_tagging(
    ...
    storage.set_object_tags_versioned(bucket, key, version_id, tags).await?;
...
/// Handle DeleteObjectTagging request
pub async fn delete_object_tagging(
    ...
    storage.delete_object_tags_versioned(bucket, key, version_id).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_put_get_delete_object_tagging() {
        ...
        let response = put_object_tagging(&storage, "test-bucket", "test-key", None, Bytes::from(xml))
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::OK);

        // Get tags
        let response = get_object_tagging(&storage, "test-bucket", "test-key", None)
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::OK);

        // Delete tags
        let response = delete_object_tagging(&storage, "test-bucket", "test-key", None)
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::NO_CONTENT);

        // Get tags should return empty now
        let response = get_object_tagging(&storage, "test-bucket", "test-key", None)
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::OK);
    ...
    #[tokio::test]
    async fn test_put_object_tagging_object_not_found() {
        let (storage, _temp) = create_test_storage().await;

        storage.create_bucket("test-bucket").await.unwrap();

        let xml = r#"<Tagging><TagSet><Tag><Key>k</Key><Value>v</Value></Tag></TagSet></Tagging>"#;
        let result = put_object_tagging(&storage, "test-bucket", "nonexistent", None, Bytes::from(xml)).await;

        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_get_object_tagging_bucket_not_found() {
        let (storage, _temp) = create_test_storage().await;

        let result = get_object_tagging(&storage, "nonexistent", "key", None).await;
